import {FoodCategories} from '../src/models/FoodCategories';
...
// POST api test
describe('Post API', () => {
  test("should create FoodCategories", async () => {
    const res = await request(app)
      .post("/FoodCategoriess")
    ...
    expect(200);
    id=res.body.data.FoodCategoriesData._id
  ...
  describe('GET api by id', () => {
    ...
    test('GET by ID api not exist', () => {
      const res = request(app)
        .get(`/FoodCategoriessnotExist/${id}`)
        .set("Authorization", `Bearer ${userOne.token}`)
      expect(400)
    })
  })
  ...
  describe('UPDATE api', () => {
    ...
    test('API is wrong', async () => {
      const res = await request(app)
        .patch(`/FoodCategoriessnotExist/${id}`)
        .set("Authorization", `Bearer ${userOne.token}`)
        .send({ 
          foodCategoryId:123,
          foodCategory:'Test',
  ...
  describe('DELETE api ', () => {
    ...
    test('DELETE api not exist', async () => {
      const res = await request(app)
        .delete(`/FoodCategoriessNotExist/${id}`)
        .expect(400)
      // console.log(res, '<-----res')
